...
const FestivalsCalendarPlus = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const tourList = await dataService.getHolidayTourProposals();
      
      // We will map lifecycle stages to our new 3 stages if they use old names
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // Find tours linked to these events or directly on this date
    // (If a tour is linked to an event, we display it on the event's start date, or we can span it. For simplicity, display it if its event is active this day, or if tour start date matches)
    const dayTours = tours.filter(t => {
      ...
      return false; // If no direct dates, skip
    });
  ...
  return (
    ...
    >
      ...
      {/* Calendar Grid */}
      <div className="card" style={{ padding: '0', overflow: 'hidden', border: '1px solid var(--glass-border)', background: 'rgba(10, 15, 29, 0.6)' }}>
                    ...
                    {cell.tours.map(tour => {
                      ...
                      return (
                        <div 
                          ...
                          title={\`\${tour.name} - \${tour.currentStage}\`}
                        >
                          <Navigation size={10} />
                          <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tour.name}</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div className="modal-content card animate-slide-up" style={{ maxWidth: '600px', background: 'var(--surface-color)', padding: '0', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '2rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.2rem' }}>Target Seats</div>
                  <div style={{ fontSize: '1rem', fontWeight: 700, color: '#fff' }}>{selectedTour.targetPax || selectedTour.sales?.targetCustomers || '-'}</div>
                </div>
                <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.2rem' }}>Projected Revenue</div>
