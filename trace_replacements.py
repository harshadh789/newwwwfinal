...
def apply_replacement(content, target, replacement, allow_multiple, step):
    if allow_multiple:
        if target not in content:
            print(f"Step {step}: Target NOT FOUND in index.html! Length of target: {len(target)}")
            return content
            ...
            for call in tool_calls:
                ...
                if filepath == index_path:
                    if name == 'replace_file_content':
                        ...
                        idx_content = apply_replacement(idx_content, t, r, am, step_index)
                    elif name == 'multi_replace_file_content':
                        chunks = args.get('ReplacementChunks', [])
                        idx_content = apply_chunks(idx_content, chunks, step_index)
        except Exception as e:
            pass

print("Final size:", len(idx_content))
